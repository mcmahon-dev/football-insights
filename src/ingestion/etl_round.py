...
def _sb_list(prefix_path: str) -> List[Dict]:
    ...
    # Storage list API expects JSON body including the prefix
...
def parse_players_json(blob: bytes) -> List[Dict]:
    ...
    for team_block in response:
        ...
        for p in players:
            ...
            # NOTE: the fixture_id is NOT in this file; derive from filename upstream
            row = {
                "season": int(SEASON),
                "round_text": ROUND,
                "team_id": team_id,
                ...
                "rating": float(games["rating"]) if games.get("rating") not in (None, "", "NaN") else None,
                "goals": goals.get("total"),
                "assists": goals.get("assists"),
                "fetched_datetime": now_iso(),
                # fixture_id will be added by caller (from filename)
            }
            out.append(row)
    return out
...
def main():
    ...
    # 1) Download fixtures.json for fixture list (optional but handy)
    # Not strictly required to build rows, since we parse per-fixture files.
    fixtures_json = None
    try:
        ...
        if e.response is not None and e.response.status_code == 404:
            ...
            raise
    total_fixtures = fixtures_json.get("results") if isinstance(fixtures_json, dict) else None
    if total_fixtures is not None:
        print(f"📈 fixtures.json indicates {total_fixtures} fixture results")

    # 2) List all per-fixture player JSONs
    ...
    for fname in fixture_files:
        # fixture_id from filename "players_<fixture_id>.json"
        try:
    ...
    # 3) Optional: Pandas touch for sanity checks / types (kept light)
    if rows_all:
        ...
        upsert_rows(
            ...
            conflict="fixture_id,player_id",   # requires PK/unique on these
            chunk_size=500,
        )
    else:
...
if __name__ == "__main__":
    # Minimal local cache so devs can inspect (optional, doesn't affect storage)
